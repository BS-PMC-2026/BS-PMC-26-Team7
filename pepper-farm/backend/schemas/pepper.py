...
class PepperUpdate(BaseModel):
    """Partial-update schema — every field is optional."""
    PepperName: Optional[str] = Field(None, min_length=1, max_length=100)
    ScientificName: Optional[str] = Field(None, max_length=150)
    HeatLevelScovilleMin: Optional[int] = None
    HeatLevelScovilleMax: Optional[int] = None
    OptimalSoilMoistureMin: Optional[float] = None
    ...
    @field_validator("PepperName")
    @classmethod
    def validate_pepper_name(cls, value: Optional[str]) -> Optional[str]:
        if value is None:
            return value
        value = value.strip()
        if not value:
