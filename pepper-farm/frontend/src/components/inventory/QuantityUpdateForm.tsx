...
function validateForm(form: FormState): string | null {
  ...
  if (form.AllocatedQuantity.trim() === '') return 'Store (allocated) quantity is required.';
...
export default function QuantityUpdateForm({ inventory, onSubmit, submitting }: Props) {
  ...
  return (
    ...
    >
      ...
      <div>
        <label htmlFor="WarehouseQuantity" className="mb-1 block text-sm font-medium">
          Warehouse Quantity *
        </label>
        <input
          ...
          min="0"
          step="1"
          value={form.WarehouseQuantity}
          onChange={handleChange}
          className="w-full rounded-md border border-gray-300 px-3 py-2 outline-none focus:border-black"
        />
        <p className="mt-1 text-xs text-gray-500">
          Actual units currently in the farm warehouse (source of truth).
        </p>
      </div>

      <div>
        <label htmlFor="AllocatedQuantity" className="mb-1 block text-sm font-medium">
        ...
        />
        <p className="mt-1 text-xs text-gray-500">
          Units exposed in the customer-facing catalog. Must be ≤ warehouse quantity.
        </p>
      </div>
